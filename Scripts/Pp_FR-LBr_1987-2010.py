...
def model(
   ...
   ## Site specific parameters from csv external file
    paraSiteFilePath = os.path.join(basePath, 'Parameters_files', 'Site' ,'FR-LBr.csv')
   ...
   ## Species parameters from csv external file
    ...
    _installation.initialTreesDimensionsFile = os.path.join(basePath, 'Parameters_files', 'Tree_stand', 'FR-LBr_dbh_1986.csv') 
...
##instantiate the model 
    mdl = model( 
        startYear = 1986,
        meteoFile = os.path.join(basePath, 'Met_files' ,'Met_FR-LBr_1984-2011.csv'),
        )
    endYear = 2010 #included
    fileoutName = os.path.join(basePath, 'Output_files', 'FR-LBr_1987-2010_dtest.csv')
